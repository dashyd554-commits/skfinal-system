...
// Fix: explicit float comparison with small epsilon tolerance
...
if (isset($_POST['approve'])) {
    if ($canAfford) {
        ...
        $stmt = $conn->prepare("UPDATE projects SET status = 'approved', budget_allocated = ? WHERE id = ?");
        $stmt->execute([$budgetRequested, $project_id]);

        $stmt = $conn->prepare("INSERT INTO budget_transactions (barangay_id, project_id, amount, description) VALUES (?, ?, ?, ?)");
        $stmt->execute([$barangay_id, $project_id, $budgetRequested, "Approved: " . $project['name']]);
...
.topbar-left p { font-size: 0.8rem; color: #94a3b8; margin-top: 3px; }
...
.btn-back:hover, .btn-print:hover { opacity: 0.85; transform: translateY(-1px); }
...
.btn-approve:hover:not(:disabled) { transform: translateY(-2px); opacity: 0.92; }
...
@media print {
    ...
    .print-header h1 { font-size: 20px; color: #1e3a5f; margin-bottom: 4px; }
    .print-header p  { font-size: 12px; color: #555; }

    .topbar-left h2 { color: #1e3a5f; font-size: 18px; }
    .topbar-left p  { color: #555; }
    ...
    .desc-block  { background: #f5f7fa !important; color: #333 !important; border: 1px solid #e0e6f0; }
    .desc-label  { color: #555 !important; }

    .kpi-mini {
        background: #f5f7fa !important;
        border: 1px solid #dce3ec !important;
    }
    .kpi-mini-label { color: #555 !important; }
    .kpi-mini-value { color: #1e3a5f !important; font-size: 1rem !important; }

    .bar-track { background: #e2e8f0 !important; }

    .status-pending  { background: #fef9c3 !important; color: #854d0e !important; border-color: #fcd34d !important; }
...
<div class="main">
    ...
    <?php if (!in_array($project['status'], ['pending', 'pending_treasurer'])): ?>
    <div class="alert alert-warning">
        ℹ️ This proposal has already been <b><?= strtoupper($project['status']) ?></b>. No further action can be taken.
    </div>
    <?php endif; ?>
            ...
            <div class="glass">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">
                        <span class="status-badge status-<?= in_array(strtolower($project['status'] ?? ''), ['pending','pending_treasurer']) ? 'pending' : strtolower($project['status'] ?? 'pending') ?>">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">
                        <?= $project['activity_date'] ? date('F d, Y', strtotime($project['activity_date'])) : '—' ?>
                    </span>
                </div>
                <?php endif; ?>
            </div>
        ...
        <div>
            ...
            <div class="glass">
                ...
                <div class="kpi-stack">
                    ...
                    <div class="kpi-mini" style="border-color:rgba(252,211,77,0.3);background:rgba(252,211,77,0.06);">
                        <div class="kpi-mini-icon">📌</div>
                        <div>
    ...
    <div class="print-signatures">
        <div class="sig-block">
            ...
            <div class="sig-role">SK Chairperson</div>
        </div>
        <div class="sig-block">
            <div class="sig-line"></div>
            <div class="sig-name">Reviewed by</div>
            <div class="sig-role">SK Treasurer</div>
        </div>
        <div class="sig-block">
            <div class="sig-line"></div>
            <div class="sig-name">Noted by</div>
            <div class="sig-role">SK Secretary</div>
        </div>
    </div>

    <div class="print-meta">
        <span>Printed: <script>document.write(new Date().toLocaleString())</script></span>
        <span>SK Decision Support System &copy; 2026</span>
